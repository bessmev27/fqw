...
class UserFolderService:
    ...
    def create_directory(self,create_directory_request):
        ...
        new_dir = Directory(create_directory_request["name"],create_directory_request["user_id"],create_directory_request["parent_id"])
        self.__db.add(new_dir)
        self.__db.commit()
        return new_dir

    def get_directory(self, user_id,dir_id = None):
        if dir_id is None:
    ...
    def create_file(self,create_file_request):
        new_file = File(create_file_request["dir_id"],create_file_request["name"],create_file_request["size"])
